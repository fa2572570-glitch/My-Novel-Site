...
export const EdgeReadAloudBar: React.FC<EdgeReadAloudBarProps> = ({
  ...
  const toggleMinimize = (min: boolean) => {
    setIsMinimized(min);
    localStorage.setItem('edge_tts_bar_minimized', min ? 'true' : 'false');
  };
  ...
  const getInitialBarPosition = (): { x: number; y: number } => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (typeof parsed.x === 'number' && typeof parsed.y === 'number') {
          return {
            x: Math.min(Math.max(8, parsed.x), window.innerWidth - 120),
            y: Math.min(Math.max(8, parsed.y), window.innerHeight - 60)
          };
        }
      }
    } catch (e) {
    ...
    const defaultX = Math.max(8, window.innerWidth - 420);
  ...
  // Keep bar within screen on resize
  useEffect(() => {
    const handleResize = () => {
      setBarPosition(prev => {
        const currentW = barAsideRef.current?.offsetWidth || 360;
        const currentH = barAsideRef.current?.offsetHeight || 50;
        return {
          x: Math.min(Math.max(8, prev.x), Math.max(8, window.innerWidth - currentW - 8)),
          y: Math.min(Math.max(8, prev.y), Math.max(8, window.innerHeight - currentH - 8))
        };
      });
    };
  ...
  const handleBarPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (session.hasMoved) {
      ...
      const barWidth = barEl ? barEl.offsetWidth : 360;
      const barHeight = barEl ? barEl.offsetHeight : 54;

      const rawX = session.initialBarX + deltaX;
      const rawY = session.initialBarY + deltaY;

      const clampedX = Math.min(Math.max(8, rawX), Math.max(8, window.innerWidth - barWidth - 8));
      const clampedY = Math.min(Math.max(8, rawY), Math.max(8, window.innerHeight - barHeight - 8));
    ...
    >
      <div className="flex flex-col items-end gap-1.5">
        ...
        <div
          ...
          className={`flex items-center gap-1 sm:gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-full shadow-2xl border backdrop-blur-xl transition-shadow duration-200 ${
            isBarDragging
        ...
        {showClockInBar && showTimelineScrubber && state.totalSentencesInChapter > 0 && onSeekToChapterSentence && (
          <div 
            ...
            className="w-[300px] sm:w-[360px] px-4 py-2.5 rounded-2xl shadow-2xl border backdrop-blur-xl text-xs flex flex-col gap-1.5 animate-in slide-in-from-top-1"
          >
