...
export function useEdgeReadAloud({
  ...
  const playChunk = useCallback(async (
    ...
    try {
      ...
      activeAudio.playbackRate = state.playbackSpeed;
  ...
  }, [getChapterPlan, getChapterSentenceIndex, getChunkMetadata, preloadStandbyChunk, startTrackingLoop, state.playbackSpeed, updateMediaSession, onChapterChange]);
  ...
  }, [currentChapterIndex, playChunk, startTrackingLoop, state.currentChapterIndex, state.isPlaying, state.playbackSpeed, updateMediaSession]);
  ...
  const setPlaybackSpeed = useCallback((newSpeed: number) => {
    ...
    if (audioARef.current) audioARef.current.playbackRate = newSpeed;
    if (audioBRef.current) audioBRef.current.playbackRate = newSpeed;
